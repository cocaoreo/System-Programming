...
bool less(const struct hash_elem *a, const struct hash_elem *b, void *aux){///hash less
  if(a->value < b->value){
    return true;
  }
...
unsigned hash_hashf(const struct hash_elem *e, void *aux){
  return hash_int(e->value);
}

void square(struct hash_elem *e, void *aux){// square
  e->value = e->value * e->value;
}

void triple(struct hash_elem *e, void *aux){// triple
  e->value = e->value * e->value* e->value;
}

void destruct(struct hash_elem *e, void *aux){// destruct
  free(e);
}

unsigned hash_int_2(int i){// hash_int_2
  return i%16;
}